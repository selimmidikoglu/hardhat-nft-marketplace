...
!developmentChains.includes(network.name)
    ...
    : describe("Nft Marketplace Unit Tests", async function () {
          ...
          beforeEach(async () => {
              ...
              const result = await basicNftContract.approve(nftMarketplace.address, TOKEN_ID)
          })
          ...
          it("list and can be bought", async () => {
              ...
              await playerConnectedNftMarketplace.buyItem(basicNftContract.address, TOKEN_ID, { value: PRICE })
              const newOwner = await basicNftContract.ownerOf(TOKEN_ID)
              const deployerProceeds = await nftMarketplace.getProceeds(deployer.address)
              assert(newOwner.toString() == user.address)
              assert(deployerProceeds.toString() == PRICE.toString())
              console.log(await nftMarketplace.getListing(newOwner.toString(), TOKEN_ID))
          })

          it("update listing", async () => {
              const updatedPrice = ethers.utils.parseEther("0.2")
              await nftMarketplace.listItem(basicNftContract.address, TOKEN_ID, PRICE)
              expect(await nftMarketplace.updateListing(basicNftContract.address, TOKEN_ID, updatedPrice)).to.emit("ItemListed")
              const listing = await nftMarketplace.getListing(basicNftContract.address, TOKEN_ID)
